/**
 * @fileoverview Unit tests for the CoinPay OAuth flow:
 * generic OAuth helpers (state/PKCE, authorize URL, token/userinfo exchange)
 * from `@profullstack/stack/coinpay`, plus the qrypt-specific username
 * derivation/uniqueness helpers kept in `src/lib/auth/coinpay.js`.
 */
...
describe('generateCoinPayState / generateCoinPayPkcePair', () => {
	it('generates distinct random state values', () => {
		expect(generateCoinPayState()).not.toBe(generateCoinPayState());
	});
...
describe('exchangeCoinPayCode (mocked fetch)', () => {
...
describe('fetchCoinPayUserinfo (mocked fetch)', () => {
	...
	it('throws when neither sub nor email present', async () => {
		const fetchImpl = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({}) });
		await expect(
			fetchCoinPayUserinfo({ issuer: 'https://x', accessToken: 'at', fetch: fetchImpl })
