...
export async function POST(request) {
  try {
    ...
    if (!conversation_id || !encrypted_contents) {
      return NextResponse.json({ error: 'Missing required fields: conversation_id, encrypted_contents' }, { status: 400 });
    }
...
export async function GET(request) {
  ...
  try {
    ...
    if (participantError || !participant) {
      console.log('🔐 [API] ❌ User not authorized for conversation:', participantError?.message);
      return NextResponse.json({ error: 'Not authorized to view messages in this conversation' }, { status: 403 });
    }
